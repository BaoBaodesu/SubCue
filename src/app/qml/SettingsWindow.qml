...
Window {
    ...
    function refreshAsrModels(preferred) {
        asrModelData = controller.asrModels(asrProvider.currentValue || "dashscope")
        asrModel.currentIndex = indexById(asrModelData, preferred)
    }
    function refreshAiModels(preferred) {
        ...
        aiCredentialStatus.text = provider
            ? controller.credentialStatus("SubCue/AI/" + provider.id) : "尚未配置"
    ...
    function loadValues() {
        ...
        const savedDevice = controller.setting("whisperDevice") || "auto"
        whisperDevice.currentIndex = savedDevice === "cpu" ? 2 : (savedDevice === "cuda" ? 1 : 0)
        ...
        asrCredentialStatus.text = savedAsr === "dashscope"
            ? controller.credentialStatus() : "尚未配置"
    ...
    onVisibleChanged: if (visible) loadValues()
    Component.onCompleted: if (typeof nativeTheme !== "undefined" && nativeTheme)
        nativeTheme.applyDarkTitleBar(settingsWindow, Theme.titleBar, Theme.text, Theme.border)
        ...
        ColumnLayout {
            ...
            ScrollView {
                ...
                GridLayout {
                    ...
                    SubButton {
                        ...
                        enabled: !settingsWindow.asrOperationBusy && asrModel.currentIndex >= 0
                        onClicked: {
                    ...
                    SubButton {
                        ...
                        onClicked: {
                            const values = {
                                ...
                                "whisperDevice": ["auto", "cuda", "cpu"][whisperDevice.currentIndex],
                    ...
                    FieldLabel { text: "API Key"; visible: aiAssist.checked && selectedProvider() && selectedProvider().authMode !== "none" }
                    SubTextField {
                        id: aiKey
                        visible: aiAssist.checked && selectedProvider() && selectedProvider().authMode !== "none"
                        enabled: !settingsWindow.aiOperationBusy
                        Layout.fillWidth: true
                        echoMode: TextInput.Password
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    SubButton { text: "取消"; onClicked: settingsWindow.hide() }
                    PrimaryButton {
                        text: "保存"
                        enabled: !settingsWindow.asrOperationBusy && !settingsWindow.aiOperationBusy
                        onClicked: {
                            const provider = selectedProvider()
